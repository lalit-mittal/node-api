...
exports.getPosts = async (req, res, next) => {
  try {
    const posts = await Post.find()
      .populate("createdBy", "name createdAt role")
      .exec();
    res.status(200).json({
...
exports.getUserPost = async (req, res, next) => {
  try {
    const posts = await Post.find({ createdBy: req.user._id })
      .populate("createdBy", "name createdAt")
      .exec();
    res.status(200).json({
      message: "Posts fetched successfully",
      data: posts,
...
exports.createPost = async (req, res, next) => {
  try {
    ...
    const post = new Post({ title, content, createdBy: req.user._id, image });
...
exports.updatePost = async (req, res, next) => {
  try {
    ...
    const post = await Post.find({ _id: postId, createdBy: req.user._id });
    if (post.length === 0) {
      const error = new Error("Unautherized Access");
      error.statusCode = 401;
      throw error;
...
exports.editComment = async (req, res, next) => {
  try {
    ...
    if (postComment.userId.toString() !== req.user._id.toString()) {
      const error = new Error("Unautherized Access");
      error.statusCode = 401;
      throw error;
    }
